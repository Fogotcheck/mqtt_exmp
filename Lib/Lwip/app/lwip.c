...
/* USER CODE BEGIN 1 */
void dhcp_netif_status_callback(struct netif *netif);
/* USER CODE END 1 */
...
int MX_LWIP_Init(void)
{
	...
	if (NULL == (lwip_event_group = xEventGroupCreate()))
		return -1;
	...
	netif_set_status_callback(&gnetif, dhcp_netif_status_callback);
...
static void ethernet_link_status_updated(struct netif *netif)
{
	if (netif_is_up(netif)) {
		...
		gw = netif->gw;
		osEventFlagsSet(lwip_event_group, LWIP_EVENT_FLAG_LINK_UP);
	} else /* netif is down */
	{
		osEventFlagsSet(lwip_event_group, LWIP_EVENT_FLAG_LINK_DOWN);
	}
}
...
void dhcp_netif_status_callback(struct netif *netif)
{
	if (netif_is_up(netif)) {
		if (dhcp_supplied_address(netif)) {
			osEventFlagsSet(lwip_event_group,
